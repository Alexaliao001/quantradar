...
final class WatchlistStore: ObservableObject {
    ...
    func canAdd() -> Bool {
        items.count < AppAccess.watchlistLimit
    }

    @discardableResult
    func add(ticker: String) -> Bool {
    ...
    func setRemind(_ ticker: String, enabled: Bool) {
        guard let idx = items.firstIndex(where: { $0.ticker == ticker }) else { return }
        items[idx].remindOnImprove = enabled
        persist()
        if enabled {
            Task { await AlertScheduler.requestPermissionAndScheduleSample(for: ticker) }
        }
    }
...
enum AlertScheduler {
    static func requestPermissionAndScheduleSample(for ticker: String) async {
