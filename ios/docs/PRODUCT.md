# QuantRadar iOS — Product & Monetization (locked)

> **Separate from web.** App Store cash register. **iOS data plane = $0 COGS** (see [DATA_PLANE.md](./DATA_PLANE.md)).

## Dual product

| | Web | iOS |
|--|-----|-----|
| Money | Stripe Pro | **Free download → $9.99 one-time unlock** |
| Data | **Massive / Polygon** when live | **Yahoo free + on-device score** |
| Fidelity | Full desk / options honesty gates | Mechanical posture only (no OPRA) |
| Cost to you | Massive bill | **~$0 per scan** |

## Monetization (iOS)

| Lever | Choice |
|------|--------|
| App price | **Free** (listing) |
| Core unlock | **Non-consumable IAP $9.99** (`one.quantradar.app.unlock`) founder price, first 1,000 |
| Free preview | Today **SPY** + **one lifetime personal ticker** (live scan, not a canned demo) |
| Locked 2nd ticker | Score is computed, card is blurred, paywall asks the anxiety question |
| Live+ | Code exists; **not sold in 1.2** until posture-change alerts prove value |
| Why people pay | One score / most days don’t act — not Massive parity |

## Depth (1.2)

- 90-day posture strip + setup forward evidence (this ticker’s own history)
- Earnings ±3d forces WAIT (never SETUP through an earnings window)
- Sector heatmap (unlocked)
- Discipline streak + wait log
- Weekday 09:25 ET briefing notification (static copy — no fake scores)
- Home Screen SPY widget
- Reverse-flex share: “Today I didn’t force a trade”

## Growth

```text
ASO → Free App Store install → live SPY + one personal ticker → 2nd ticker blur → $9.99 unlock
```

**CAC rule:** Unlock net ≈ $7 after Apple cut. Do **not** spray mass retail CPC.

## Honesty

- Do not market iOS as “same engine as web Massive.”
- User-facing copy talks posture, not COGS / data-source brands.
- Educational only — not investment advice. Action label is **SETUP**, not BUY.
- No fake countdown, fake inventory, or invented scores in notifications.
- Fail-closed when bars unavailable.
- iOS legal: `/privacy-ios` and `/terms-ios` (not the web OAuth/Stripe policy).

## ASC note

Public listing must be **free + Unlock**, not paid download. See [APP_STORE_LAUNCH.md](./APP_STORE_LAUNCH.md).
